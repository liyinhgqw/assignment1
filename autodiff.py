...
class MulOp(Op):
    ...
    def compute(self, node, input_vals):
        ...
        """TODO: Your code here"""
        assert len(input_vals) == 2
        assert np.shape(input_vals[0]) == np.shape(input_vals[1])
        return input_vals[0] * input_vals[1]
        ...
        for node in topo_order:
            if node.inputs:
                ...
                for in_node in node.inputs:
                    in_vals.append(node_to_val_map[in_node])
                node_to_val_map[node] = node.op.compute(node, in_vals)
    ...
    for node in reverse_topo_order:
        if node.inputs:
            ...
            for i, in_node in enumerate(node.inputs):
                if in_node in node_to_output_grad:
                    node_to_output_grad[in_node] += in_grads[i]
                else:
                    node_to_output_grad[in_node] = in_grads[i]
